...
void GPUIMAGE::Allocate()
{
    ...
    data.buf   = wgpuctx::CreateStorage(Bytes());
    data.ptr   = data.buf.Get();
    wgpuctx::Zero(data.buf, Bytes());
}
...
void GPUIMAGE::FillBuffer(float val)
{
    ...
    wgpuctx::Upload(data.buf, v.data(), Bytes());
}
...
void GPUIMAGE::DuplicateFromCUDAImage(GPUIMAGE::Pointer cp)
{
    ...
    // iteration.
    wgpuctx::CopyBuffer(data.buf, cp->getFloatdata().buf, Bytes());
}
...
void GPUIMAGE::SetImageFromITK(ImageType3D::Pointer itk_image, bool)
{
    ...
    Allocate();
    wgpuctx::Upload(data.buf, itk_image->GetBufferPointer(), Bytes());
}
...
void GPUIMAGE::SetImageFromITK(DisplacementFieldType::Pointer itk_field)
{
    ...
    wgpuctx::Upload(data.buf, host.data(), Bytes());
}
...
GPUIMAGE::ImageType3D::Pointer GPUIMAGE::CudaImageToITKImage()
{
    ...
    wgpuctx::Download(data.buf, buf, nv * sizeof(float));
...
GPUIMAGE::DisplacementFieldType::Pointer GPUIMAGE::CudaImageToITKField()
{
    ...
    // buffer size, which Dawn rejects and Download turns into _Exit(1) - i.e. the
    // port was LESS defensive than the reference, which CLAUDE.md 0.0 forbids in
    ...
    const size_t nv = NumVoxels();
    std::vector<float> host(nv * 3);
    wgpuctx::Download(data.buf, host.data(), host.size() * sizeof(float));
