...
// GPUIMAGE - the Metal counterpart of CUDAIMAGE, with the same public surface.
//
// Under USEMETAL this is typedef'd to CUDAIMAGE at the bottom of the file, so
...
//   * CreateTexture() is a no-op. Sampling is done manually over the same storage
//     buffer, as in the WebGPU backend and for the same reason (CLAUDE.md 5.2)
...
public:
    ...
    void Allocate();
    // Allocate with the contents already in place: no zero-fill to overwrite, no
    // queue drain. For the paths that upload a whole image straight after allocating.
    void AllocateFrom(const void *src);
    ...
    // No sampler is used (the reference needs clamp-to-border), so the "texture"
