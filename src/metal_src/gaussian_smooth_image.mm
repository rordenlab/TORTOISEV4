#ifndef _MTL_GAUSSIANSMOOTH_CXX
#define _MTL_GAUSSIANSMOOTH_CXX

#include "itkGaussianOperator.h"
#include "gaussian_smooth_image.h"
#include "metal_context.h"
#include "gaussian_smooth_image.metal.h"
#include <stdexcept>
                                               ...
                                               const std::vector<float> &tapv, float std)
{
    ...
    mtlctx::Buffer buf1 = mtlctx::CreateStorage(bytes);
    mtlctx::Buffer buf2 = mtlctx::CreateStorage(bytes);
    ...
    struct { uint32_t axis; mtlctx::Buffer in, out; } passes[3] = {
    ...
    for(int s = 0; s < 3; s++)
    {
        mtlctx::ComputePipeline pipe =
            mtlctx::Pipeline("gaussian_smooth_image", kgaussian_smooth_imageMSL, "main_smooth",
                              wgx, wgy, wgz, {{"AXIS", (double)passes[s].axis}});
        mtlctx::Dispatch(pipe, {passes[s].in, passes[s].out, params}, gx, gy, gz);
    }
    ...
    if(main_image->components_per_voxel == 3)
    {
        ...
        p.wts[0] = weight1;
        p.wts[1] = 1.0 - weight1;
        mtlctx::Buffer bparams = mtlctx::CreateUniform(&p, sizeof(p));
        mtlctx::ComputePipeline pipe =
            mtlctx::Pipeline("gaussian_smooth_image", kgaussian_smooth_imageMSL,
